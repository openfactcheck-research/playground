...
import { fetchRunApi, startRunApi } from '@/services/runs.api'
...
export function useRuns() {
  ...
  async function pollRun(projectId: string, workspaceId: string): Promise<void> {
    try {
      const run = await fetchRunApi(projectId, workspaceId)
  ...
  async function executeRun(projectId: string, workspaceId: string, pipeline: object): Promise<void> {
    stopPolling()
    try {
      const run = await startRunApi(projectId, workspaceId, pipeline)
      currentRun.value = run
      pollTimer = setInterval(() => {
        void pollRun(projectId, workspaceId)
      }, POLL_INTERVAL_MS)
    }
    catch {
