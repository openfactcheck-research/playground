/**
 * Claim Processor Block
 * Processes claims using various fact-checking frameworks or custom prompts
 */
import * as Blockly from 'blockly/core'
import { Order, pythonGenerator } from 'blockly/python'
import { PALETTE } from '@/blockly/theme'

export const BLOCK_TYPE = 'claim_processor'
...
export function register(): void {
  Blockly.Blocks[BLOCK_TYPE] = {
    init(this: Blockly.Block): void {
      ...
      this.appendDummyInput()
        ...
        .appendField(
          new Blockly.FieldDropdown(FRAMEWORK_OPTIONS, function (
            ...
            if (block) {
              ;(block as ClaimProcessorBlock).updatePromptInput_(newValue === 'custom')
            }
            return newValue
          }),
          'FRAMEWORK',
      ...
      this.setTooltip('Process claims using a fact-checking framework or custom prompt')
      this.setHelpUrl('')
    },
     ...
     */
    updatePromptInput_(this: ClaimProcessorBlock, showPrompt: boolean): void {
  ...
  pythonGenerator.forBlock[BLOCK_TYPE] = function (
    ...
    // Generate processor code as statement
    let code: string

    if (framework === 'custom') {
      code = `result = process_claim(
    claim=claim,
    framework="${framework}",
    model=${model},
    prompt=${prompt}
...
type ClaimProcessorBlock = Blockly.Block & {
